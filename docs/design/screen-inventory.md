...
| `/budget` | active | Maand- en weeksturing plus budgetbeheer. | Zien waar ruimte zit en budgetinstellingen aanpassen. | Scheiding tussen dag/week/maand, beheersegment, budgetrekenregels, overzichtelijke voortgang en toegang tot een voorstel-eerst budgetflow met een slimme strategiekaart-selector. | budget plan, budgetsettings, reserve rules, maandopties, categorieën, rekeningen, transacties, scope preference | loading: budget skeleton; empty: nog geen budgetplan; partial: settings of forecast deels beschikbaar; error: retry + fallback summary |
...
| `/budget/setup` | utility | Begeleide budget-instelflow. | Een slimme strategie kiezen, een voorstel laten maken, bevestigen of verfijnen en lokaal per blok bijsturen. | `Start` met strategiekaarten (`Normaal`, `Balans`, `Bespaarmodus`; `Balans` standaard), `Analyse`, `Voorstel`, `Verfijnen`, `Onderdelen bewerken` en `Review na toepassen` blijven binnen dezelfde flow; AI start niet als chat-first entry; dezelfde selector staat ook in de admin design-system catalogus en valt op kleine schermen terug naar een slider. | budget plan, budget coach, budget settings, reserve rules, income preview, forecast summary | loading: analyse/progress; empty: nog geen voorstel of onvoldoende brondata; partial: voorstel met beperkte brondata; error: herstelactie of terug naar Budget; success: voorstel toegepast en review-state met finetune-routes |
| `/budget/setup/proposal` | utility | Stap-voor-stap opzet binnen de slimme setup wizard. | In vier blokken inkomen, bescherming, resterende ruimte en budgetverdeling klaarmaken. | Strategie blijft secundair (geen dominante chiplaag), componentaanpassingen gebeuren per blok en primaire flow gaat door naar review. | budget setup orchestrator, budget plan context, reserve/income drafts | loading: voorstel wordt opgebouwd; empty: te weinig data; partial: lage confidence; error: retry; success: route naar review |
| `/budget/setup/review` | utility | Besluitreview vóór toepassen. | Begrijpen wat het plan betekent en met één hoofdactie bevestigen. | Drie betekeniskaarten in vaste volgorde en CTA-paar `Gebruik dit plan` / `Nog iets aanpassen`; review voelt niet als changelog. | proposal review context, apply service, geselecteerde maand/mode | loading: n.v.t.; empty: ontbreken reviewcontext -> terug naar proposal; partial: beperkte samenvatting; error: apply fout + herstelactie |
