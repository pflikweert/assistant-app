...
async function getLatestStartingBalance(monthStartIso: string) {
  ...
  for (const row of (data || []) as any[]) {
    const balance = parseSaldoValue((row.metadata || {}) as Record<string, unknown>);
...
function descriptor(tx: Pick<ForecastTx, "counterparty" | "details">) {
  return normalizePattern(tx.counterparty || tx.details.split("|")[0] || tx.details);
}
...
function buildVariableBucket(
  ...
  const haystack = normalizePattern(`${tx.counterparty || ""} ${tx.details || ""}`);
...
export async function recomputeCurrentMonthCashflowForecast(reference = new Date()) {
  ...
  const [categoryMap, monthTransactions, historyTransactions, incomeSources, startingBalance] =
    await Promise.all([
      fetchCategoryMap(),
      fetchTransactionsInRange(monthStartIso, monthEndIso),
      fetchTransactionsInRange(dateToIso(subtractDays(monthStart, 90)), monthEndIso),
      fetchIncomeSources(),
  ...
  for (const source of incomeSources) {
    ...
    if (!frequencyAppliesInMonth(source.income_frequency, anchorDate, monthStart)) {
      continue;
    }
  ...
  for (const recurringItem of recurringGroups.values()) {
    if (!frequencyAppliesInMonth(recurringItem.recurringType, recurringItem.anchorDate, monthStart)) {
      continue;
    }
  ...
  for (const tx of historyTransactions) {
    ...
    const categoryKey = categoryId ? categoryMap.get(categoryId)?.key || null : null;
  ...
  const avgGroceries =
    variableStats.groceries.count > 0
      ...
      : 0;
  const avgFuel =
    variableStats.fuel.count > 0 ? variableStats.fuel.sum / variableStats.fuel.count : 0;
  const avgSmoking =
    variableStats.smoking.count > 0
      ? variableStats.smoking.sum / variableStats.smoking.count
      : 0;
  const avgOtherVariable =
    variableStats.other.count > 0 ? variableStats.other.sum / variableStats.other.count : 0;

  const expectedVariableCosts = avgGroceries + avgFuel + avgSmoking + avgOtherVariable;
